 ...
 * 记录会话活动
 */
async function logSessionActivity(agentName, activity, details = {}) {
  try {
    ...
    await require('./agent-manager').logMessage('session', agentName, message, 'activity');
    return true;
  } catch (error) {
 ...
 * 检查配置是否需要更新
 */
async function checkConfigUpdateNeeded() {
  try {
